...
public class CreditoBean implements Serializable {
	...
	public void calcularCredito() {
		if (!comprobarCredito()) {
			...
			credito.setDetallesCreditos(new ArrayList<DetalleCredito>());
			for (int i = 1; i <= credito.getMeses(); i++) {
...
//		listaGarantes.add(personaService.obtenerPorId(garante.getId()));
	}
	...
	public void obtenerGarantesProBusqueda() {
		if (criterioBusqueda.length() >= 0 && criterioBusqueda.length() <= 3)
			...
			// listaGarantesBusqueda = personaService.obtenerPorCriterioPorRol(
			// criterioBusqueda.toUpperCase(), Rol.CLIE);
