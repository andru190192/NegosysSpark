...
public class FacturaBean implements Serializable {
	...
	//
	// if (compareTo(pagoEntrada.getCuota(), cantidadFactura.getValorTotal()) >
	// 0)
	...
	public void cargarCliente(Persona persona) {
		Persona p = clienteService.obtenerObjeto("", new Object[] {}, false, new Object[] {});
		// obtenerPorPersonaId(persona.getId());
		factura.setCliente(p);
		factura.setClienteFactura(p);
		cliente = p
				.getId()
				.toString()
				.concat("-")
...
//		}
	...
	public void ingresarDetalleProducto() {
		...
		if (insertar) {
			Integer cantidadActual = kardexService.obtenerSaldoActual(
					producto.getCodigo(), localId).getCantidad();
			if (cantidadActual == 0) {
				...
				if (pvp == null) {
					...
					cantidadFactura = facturaService.cargarProductoLista(
							estadoProducto, cantidad, producto.getCodigo(),
							listaFacturaReporte, localId, pvp);
	...
	public void insertarFactura() {
		if (cantidadFactura.getValorTotal().compareTo(new BigDecimal("19")) > 0
				&& factura.getClienteFactura().getNumeroDocumento()
	...
	public void nuevaFactura() {
		// FACTURA
		factura = new Factura();
		factura.setCliente(new Persona());
		factura.setClienteFactura(new Persona());
		factura.setVendedor(new Persona());
		...
		localId = localService.obtenerObjeto("", new Object[] {}, false,new Object[] {})
				.getId();
	...
	public void obtenerClientes() {
		if (Utils.validacion(criterioCliente, "(c|C){1,1}[0-9]+", "")) {
			listaClientes = clienteService.obtenerLista("", new Object[] {},
					false,new Object[] {});
			// obtener(null,
	...
	public void obtenerClientesFactura() {
		...
		if (listaClientes.size() == 1) {
			...
			// .getId());
			factura.setClienteFactura(p);
			clienteFactura = p.getId().toString().concat("-")
					.concat(p.getNumeroDocumento()).concat("-")
					.concat(p.getRazonSocial());
			RequestContext.getCurrentInstance().execute(
	...
	public void obtenerProducto() {
		// if (Utils.validacion(criterioProducto, "(c|C){1,1}[0-9]+", ""))
