...
public class UtilsFactura {
	...
	private static FacturaE armarFacturaXML(Factura factura, String cAcceso,
		...
		Persona p = UtilsFactura.personaService.obtenerObjeto("",
				new Object[] {}, false, new Object[] {});
		// obtenerPorPersonaId(factura
...
//		for (DetalleFactura df : list)
		...
		// .subtract(df.getDescuento()), Tarifa.IC001,
		//						df.getProducto().getId()));
		return detalles;
	}
	...
	public static FacturaE facturaXML(Factura factura, CantidadFactura cf,
		...
		Local local = UtilsFactura.localService.obtenerObjeto("", new Object[] {}, false, new Object[] {});
	...
	private static InfoFactura infoFactura(Factura f, Persona p,
		...
		infoFactura
				.setTipoIdentificacionComprador(tipoIdentificacionComprador(p
						.getNumeroDocumento()));
		infoFactura.setRazonSocialComprador(p.getRazonSocial());
		infoFactura.setIdentificacionComprador(p.getNumeroDocumento());
		infoFactura
	...
	private static BigDecimal totalImpuestoIce(
		...
		for (DetalleFactura de : list) {
			Tarifa t = null;
			// tarifaService.obtenerPorFechaProducto(fechaFactura, de
			// .getProducto().getId(),
