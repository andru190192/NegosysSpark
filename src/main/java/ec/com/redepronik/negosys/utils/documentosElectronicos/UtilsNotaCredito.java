...
public class UtilsNotaCredito {
	...
	private static NotaCreditoE armarNotaCreditoXML(NotaCredito notaCredito,
		...
		Persona p = UtilsNotaCredito.personaService.obtenerObjeto("",
		...
		notaCreditoE.setInfoNotaCredito(infoNotaCredito(notaCredito, p, cf,
				local));
		notaCreditoE.setDetalles(detalles(notaCredito.getFechaEmision(),
				notaCredito.getDetallesNotasCreditos()));
	...
	private static Detalle detalle(String codigoPrincipal, String descripcion,
		...
		for (Tarifa t : impuestos)
			if (t.getImpuesto() == ec.com.redepronik.negosys.entity.Impuesto.IC)
				ice = multiplicarDivide(precioTotalSinImpuesto,
						t.getPorcentaje());

		for (Tarifa t : impuestos) {
			if (t.getImpuesto() == ec.com.redepronik.negosys.entity.Impuesto.IV)
				i.getImpuesto().add(
						impuesto(
								t.getImpuesto().getId(),
								t.getIdSri(),
	...
	private static InfoNotaCredito infoNotaCredito(NotaCredito nc, Persona p,
		...
		infoNotaCredito
				.setFechaEmision(fechaFormatoString(nc.getFechaEmision()));
		...
		infoNotaCredito
				.setTipoIdentificacionComprador(tipoIdentificacionComprador(p
						.getNumeroDocumento()));
		infoNotaCredito.setRazonSocialComprador(p.getRazonSocial());
		infoNotaCredito.setIdentificacionComprador(p.getNumeroDocumento());
		...
		BigDecimal ice = totalImpuestoIce(totalConImpuestos,
				nc.getDetallesNotasCreditos(), date(nc.getFechaEmision()));
	...
	public static NotaCreditoE notaCreditoXML(NotaCredito notaCredito,
		...
		Local local = UtilsNotaCredito.localService.obtenerObjeto("",
				new Object[] {}, false, new Object[] {});
	...
	private static BigDecimal totalImpuestoIce(
		...
		for (DetalleNotaCredito dnc : list) {
			Tarifa t = null;
			// tarifaService.obtenerPorFechaProducto(fechaNotaCredito,
			// dnc.getProducto().getId(),
			// ec.com.redepronik.negosys.entity.Impuesto.IC);
			if (t != null) {
