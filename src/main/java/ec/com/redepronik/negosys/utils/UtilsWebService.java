...
public class UtilsWebService {
	...
	private static String redepronikWS = "http://ws.redepronik.com.ec/redepronikWS/";
	...
	public static void almacenamiento(String nombreArchivo, File xml, File ride) {
		...
		restTemplate.exchange(redepronikWS + "almacenamiento", HttpMethod.POST,
				new HttpEntity<MultiValueMap<String, Object>>(map, headers),
				String.class);
	}

	public static RespuestaComprobante autorizacionComprobante(
		...
		try {
			...
			AutorizacionComprobantesService service = new AutorizacionComprobantesService(
					new URL((ambiente ? produccion : prueba) + autorizacion),
					new QName("http://ec.gob.sri.ws.autorizacion",
	...
	public static RespuestaLote autorizacionComprobanteLote(
	...
	private static void certificadoSeguridadSRI() {
		String rutaCertificado = ambiente ? getRutaCertificadoSeguridadSRIProduccion()
				: getRutaCertificadoSeguridadSRIPruebas();
	...
	public static Date fechaCorte(String ruc) {
		...
		map.put("ruc", ruc);
		String respuesta = restTemplate.getForObject(redepronikWS
				+ "fechaCorte?ruc={ruc}", String.class, map);
	...
	public static RespuestaSolicitud validarComprobante(byte[] xml) {
		try {
			...
			RecepcionComprobantesService service = new RecepcionComprobantesService(
					new URL((ambiente ? produccion : prueba) + recepcion),
					new QName("http://ec.gob.sri.ws.recepcion",
