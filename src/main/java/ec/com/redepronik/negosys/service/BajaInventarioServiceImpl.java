...
import ec.com.redepronik.negosys.dao.BajaInventarioDao;
...
import ec.com.redepronik.negosys.entity.MotivoBaja;
...
@Service
public class BajaInventarioServiceImpl extends
		GenericServiceImpl<BajaInventario, Integer> implements
	...
	private boolean añadirFRLista(BajaInventarioReporte fr,
		...
		Producto p = null;
		// productoService.obtenerPorEan(fr.getCodigo());
		if (p != null) {
			Kardex k = kardexService.obtenerSaldoActual(p.getCodigo(), localId);
			if (k.getCantidad() > 0) {
				fr.setProducto(p);
	...
	public BajaInventarioReporte asignar(
		...
		Producto p = null;
		// productoService.obtenerPorProductoId(bajaInventarioReporte
		// .getProducto().getId());
		...
		if (p != null) {
			...
			// fr.getCantidad(), p.getProductosUnidades()));
	...
	public boolean comprobarCantidadFinal(List<DetalleBajaInventario> list,
			int bodegaId) {
		boolean bn = true;
		// for (DetalleBajaInventario de :
		// sumarCantidades(duplicarDetalleBajaInventario(list))) {
	...
	public boolean convertirListaBajaInventarioReporteListaBajaInventarioDetalle(
		...
		boolean bn = true;
		// bajaInventario
		// .setDetallesBajasInventarios(new ArrayList<DetalleBajaInventario>());
		// for (BajaInventarioReporte f : quitarProductosCantidadCero(list)) {
		// bn = false;
		// Producto producto = productoService.obtenerPorProductoId(f
		// .getProducto().getId());
		// DetalleBajaInventario detalleBajaInventario = new
		// DetalleBajaInventario();
	...
	public BajaInventario insertar(BajaInventario bajaInventario) {
		bajaInventario.setFechaEmision(timestamp());
		bajaInventario.setBodeguero(personaService.obtenerObjeto("",
		...
		Local local = localService.obtenerObjeto("", new Object[] {}, false,
				new Object[] {});
		// obtenerPorEstablecimiento(bajaInventario
		// .getEstablecimiento());

		generarKardex(bajaInventario.getDetallesBajasInventarios(),
	...
	public List<BajaInventario> obtener(Boolean activo, int tipoDocumentoId) {
		List<BajaInventario> list = bajaInventarioDao
				.obtenerLista(
						...
						new Object[] { tipoDocumentoId }, false,
						new Object[] {});
		if (list.isEmpty())
			presentaMensaje(FacesMessage.SEVERITY_INFO,
	...
	public BajaInventario obtenerPorBajaInventarioId(Integer bajaInventarioId) {
		BajaInventario bajaInventario = (BajaInventario) this.obtenerObjeto(
				"select distinct bi from BajaInventario bi "
						+ "left join fetch bi.detalleBajaInventario "
						+ "where bi.id=?1", new Object[] { bajaInventarioId },
				false, new Object[] {});
		return bajaInventario;
	}
