...
import ec.com.redepronik.negosys.dao.NotaCreditoDao;
import ec.com.redepronik.negosys.daoAux.EgresoCajaDao;
...
@Service
public class NotaCreditoServiceImpl extends
		GenericServiceImpl<NotaCredito, Integer> implements NotaCreditoService {
	...
	@Autowired
	private PersonaService empleadoService;
	...
	private void actualizarCantidadCuadre(NotaCredito notaCredito,
		...
		for (DetalleNotaCredito dnc : notaCredito.getDetallesNotasCreditos()) {
	...
	private boolean añadirFRLista(FacturaReporte fr, List<FacturaReporte> list,
		...
		// if (p != null) {
		// int precioId = productoService
		// .obtenerPvpPorProducto(p.getPrecios());
		...
		// p.getPrecios(), k, bodegaId, fr.getPrecioId()));
		//
		// list.set(fila, fr);
		// return true;
		// }
	...
	public FacturaReporte asignar(FacturaReporte facturaReporte) {
		// Producto producto =
		// productoService.obtenerPorProductoId(facturaReporte
		// .getProducto().getId());
		// FacturaReporte fr = new FacturaReporte();
		// fr.setProducto(producto);
		// fr.setCodigo(producto.getEan());
	...
	public CantidadFactura calcularCantidadFactura(CantidadFactura cf,
		...
		// if (pt.getTarifa().getIdSri().compareToIgnoreCase("2") == 0)
		// cf.setSt12(cf.getSt12().add(total));
		// else if (pt.getTarifa().getIdSri().compareToIgnoreCase("0") == 0)
		// cf.setSt0(cf.getSt0().add(total));
		// else if (pt.getTarifa().getIdSri().compareToIgnoreCase("6") == 0)
		// cf.setStNoObjetoIva(cf.getStNoObjetoIva().add(total));
		// else if (pt.getTarifa().getIdSri().compareToIgnoreCase("7") == 0)
		...
		BigDecimal iva = Tarifa.IV002.getPorcentaje();
	...
	public boolean convertirListaFacturaReporteListaFacturaDetalle(
		...
		boolean bn = true;
		notaCredito
				.setDetallesNotasCreditos(new ArrayList<DetalleNotaCredito>());
		...
		//
		// notaCredito.addDetallesNotasCreditos(dnc);
		// }
		if (bn)
			presentaMensaje(FacesMessage.SEVERITY_ERROR,
	...
	private void generarKardexNotaCredito(
		...
		for (DetalleNotaCredito de : sumarCantidades(detalleNotaCreditos)) {
			if (de.getProducto().getKardex()) {
				...
				kardex.setPrecio(de.getProducto().getPrecioNeto());
	...
	public NotaCredito insertar(NotaCredito notaCredito, Local local) {
		notaCredito.setFechaEmision(timestamp());
		...
		generarKardexNotaCredito(
				duplicarDetalleNotaCredito(notaCredito
	...
	public List<NotaCredito> obtener(Boolean activo, int tipoDocumentoId) {
		List<NotaCredito> list = notaCreditoDao.obtenerLista("",
	...
	public List<Factura> obtener(Integer criterioBusquedaEstado,
		...
		if (criterioBusquedaEstado == 0
			...
			if (criterioBusquedaCliente.compareTo("") != 0) {
				...
				List<Factura> list1 = facturaDao.obtenerLista("",
						new Object[] {}, false, new Object[] {});
				// .obtenerPorHql(
				// "select distinct f from Factura f "
				// + "inner join fetch f.notaCredito nc "
	...
	public List<NotaCredito> obtenerDetalleNotaCreditoPorEstado(
		...
		if (fechaInicio == null && fechaFin == null && estado == 0)
			...
			list = notaCreditoDao.obtenerLista("", new Object[] {}, false,
					new Object[] {});
			// .obtenerPorHql(
	...
	public NotaCredito obtenerPorCodigo(String codigo) {
		...
		List<NotaCredito> lista = notaCreditoDao.obtenerLista("",
				new Object[] {}, false, new Object[] {});
		// obtenerPorHql(
	...
	public NotaCredito obtenerPorNotaCreditoId(Integer notaCreditoId) {
		return (NotaCredito) facturaDao.obtenerLista("", new Object[] {},
				false, new Object[] {});
		// .obtenerPorHql(
		// "select distinct f from Factura f "
		// + "inner join fetch f.notaCredito nc "
	...
	public List<Factura> obtenerTodos() {
		return facturaDao.obtenerLista("", new Object[] {}, false,
				new Object[] {});
		// obtenerPorHql("select distinct f from Factura f "
		// + "inner join fetch f.notaCredito nc "
		// + "inner join fetch nc.detalleNotaCredito "
	...
	public List<EgresoCaja> reporteEgresoCaja(int cajero, Date fecha) {
		if (fecha == null)
			...
			return egresoCajaDao.obtenerLista("", new Object[] {}, false,
					new Object[] {});
		// .obtenerPorHql(
