...
import ec.com.redepronik.negosys.dao.FacturaDao;
import ec.com.redepronik.negosys.daoAux.CantidadDao;
import ec.com.redepronik.negosys.daoAux.GananciaFacturaReporteDao;
import ec.com.redepronik.negosys.daoAux.IngresoCajaDao;
import ec.com.redepronik.negosys.daoAux.VolumenVentaProductoDao;
...
import ec.com.redepronik.negosys.entityAux.CantidadesEntradaReporte;
...
import ec.com.redepronik.negosys.entityAux.Pvp;
import ec.com.redepronik.negosys.entityAux.VolumenVentaProducto;
import ec.com.redepronik.negosys.utils.NumerosPorLetras;
...
public class FacturaServiceImpl extends GenericServiceImpl<Factura, Integer>
	...
	public FacturaReporte asignar(DetalleFactura de) {
		...
		// productoService.obtenerPorProductoId(de
		// .getProducto().getId());
		...
		// fr.getCantidad(), producto.getProductosUnidades()));
		//
	...
	public NumeroCuotasReporte calcularNumeroCuotasEntradasCreditos(
		...
		if (factura.getCredito() != null
				&& factura.getCredito().getDetallesCreditos() != null)
			for (DetalleCredito dc : factura.getCredito().getDetallesCreditos()) {
				if (dc.getPagado())
	...
	public CantidadFactura cargarProductoLista(String codigoEspecial,
		...
		Producto p = null;
		// productoService.obtenerPorEan(ean);
		Kardex k = kardexService.obtenerSaldoActual(p.getCodigo(), bodegaId);
		FacturaReporte fr = new FacturaReporte();
		fr.setProducto(p);
	...
	public boolean comprobarCantidadFinal(List<DetalleFactura> list,
		...
		boolean bn = true;
		// for (DetalleFactura de :
		// sumarCantidades(duplicarDetalleFactura(list))) {
		// Integer localId = localService.obtenerObjeto("", new Object[] {},
		// true).getId();
	...
	private FacturaReporte convertirProductoFR(Producto p) {
		...
		// Precio tpp = productoService.obtenerValorPvp(p.getPrecios());
	...
	public NotaCredito eliminar(Factura factura, String login, String pass) {
		...
		Local local = localService.obtenerObjeto("", new Object[] {}, true,
				new Object[] {});
		...
		for (DetalleFactura de : factura.getDetalleFactura())
			if (kardexService.obtenerSaldoActual(de.getProducto().getCodigo(),
					local.getId()) == null) {
				bn = false;
				presentaMensaje(FacesMessage.SEVERITY_ERROR,
	...
	public boolean imprimirFactura(int facturaId, List<FacturaImprimir> list) {
		if (facturaId == 0)
			...
			if (factura == null) {
				...
				// factura.getClienteFactura().getPersona();
				...
				list.add(new FacturaImprimir(factura, cliente, telefono,
	...
	public Factura insertar(Factura factura,
		...
		if (comprobarCantidadFinal(factura.getDetalleFactura(),
			...
			factura.setCajero(personaService.obtenerObjeto("", new Object[] {},
					true, new Object[] {}));
			// obtenerPorDocumentoYRol(cedula,
			// Rol.CAJA));
	...
	public Long obtenerCantidadVendidaPorProveedorCliente(Integer clienteId,
		...
		// cantidadDao.obtenerLista("", new Object[] {},
		// true);
		// .obtenerPorHql(
	...
	public List<Factura> obtenerEntregaProducto(Date fechaInicio,
		...
		return null;
		// facturaDao.obtenerLista("", new Object[] {}, true, new Object[] {});
		// obtenerPorHql(
		...
		if (criterioBusquedaCliente.compareToIgnoreCase("") == 0
			...
			if (criterioBusquedaCliente.length() >= 1
				...
				if (criterioBusquedaCliente.compareTo("") != 0
					...
					list = facturaDao.obtenerLista("", new Object[] {}, false,
							new Object[] {});
				// .obtenerPorHql(
				// "select distinct f from Factura f "
	...
	public List<GananciaFacturaReporte> obtenerGananciaBrutaPorFechas(
		...
		if (fechaInicio == null || fechaFin == null)
			...
			List<Factura> listFactura = facturaDao.obtenerLista("",
	...
	public List<GananciaFacturaReporte> obtenerGananciaNetaPorFechas(
		...
		if (fechaInicio == null || fechaFin == null)
			...
			return null;
		// gananciaFacturaReporteDao.obtenerLista("", new Object[] {},
		// false);
		// .obtenerPorHql(
	...
	public GananciaFacturaReporte obtenerGananciaPorFactura(Integer egresoId,
		...
		return (GananciaFacturaReporte) gananciaFacturaReporteDao.obtenerLista(
				"", new Object[] {}, false, new Object[] {});
		// .obtenerPorHql(
	...
	public List<Factura> obtenerNoPagadosPorCiudad(int ciudadId) {
		List<Factura> list = facturaDao.obtenerLista("", new Object[] {},
				false, new Object[] {});
		// obtenerPorHql(
		// "select distinct f from Factura f "
		// + "inner join fetch f.detalleFactura df "
	...
	public Factura obtenerPorCodigo(String codigo) {
		...
		Factura egreso = null;
		// List<Factura> lista = facturaDao.obtenerLista("", new Object[] {},
		// false);
		// .obtenerPorHql(
	...
	public List<Factura> obtenerPorFechasAndVendedorAndEstadoDocumento(
		...
		if (fechaInicio != null && fechaFin != null) {
			if (vendedorId == 0 && estadoDocumento == 0)
				list = facturaDao.obtenerLista("", new Object[] {}, false,
						new Object[] {});
			// obtenerPorHql("select f from Factura f "
	...
	public List<Factura> obtenerTodos() {
		return null;
		// (List<Factura>) facturaDao.obtenerLista("", new Object[] {},
		// false);
		// obtenerPorHql(
		// "select distinct f from Factura f "
		// + "inner join fetch f.detalleFactura df "
	...
	public List<VolumenVentaProducto> obtenerVolumenVentaProducto(
		...
		if (fechaInicio == null && fechaFin == null
			...
			// list = volumenVentaProductoDao.obtenerLista("", new Object[] {},
			// false);
			// obtenerPorHql(
	...
	public List<IngresoCaja> reporteIngresoCaja(int cajero, Date fecha) {
		if (fecha == null)
			...
			return ingresoCajaDao.obtenerLista("", new Object[] {}, false,
					new Object[] {});
		// .obtenerPorHql(
